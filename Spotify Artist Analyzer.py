...
def plot_box_plot(tracks_df):
    categories = avg_tracks_df.columns
    artists = list(tracks_df.Artist.unique())
    colors=['red', 'blue', 'green', 'yellow', 'pink', 'orange']

    fig = make_subplots(rows=3, cols=3, start_cell='top-left', shared_yaxes=True, shared_xaxes=True,
        subplot_titles=([col for col in categories]), vertical_spacing=.075)
    ...
    for i, artist in enumerate(artists):
        ...
        #danceability
        fig.add_trace(go.Box(y=artist_features.danceability, name=artist, fillcolor=colors[i], line={'color':colors[i]}, opacity=.5), row=1,col=1)
        #energy
        fig.add_trace(go.Box(y=artist_features.energy, name=artist, fillcolor=colors[i], line={'color':colors[i]}, opacity=.5), row=1,col=2)
        #loudness
        fig.add_trace(go.Box(y=artist_features.loudness, name=artist, fillcolor=colors[i], line={'color':colors[i]}, opacity=.5), row=1,col=3)
        #speechiness
        fig.add_trace(go.Box(y=artist_features.speechiness, name=artist, fillcolor=colors[i], line={'color':colors[i]}, opacity=.5), row=2,col=1)
        #acousticness
        fig.add_trace(go.Box(y=artist_features.acousticness, name=artist, fillcolor=colors[i], line={'color':colors[i]}, opacity=.5), row=2,col=2)
        #instrumentalness
        fig.add_trace(go.Box(y=artist_features.instrumentalness, name=artist, fillcolor=colors[i], line={'color':colors[i]}, opacity=.5), row=2,col=3)
        #liveness
        fig.add_trace(go.Box(y=artist_features.liveness, name=artist, fillcolor=colors[i], line={'color':colors[i]}, opacity=.5), row=3,col=1)
        #valence
        fig.add_trace(go.Box(y=artist_features.valence, name=artist, fillcolor=colors[i], line={'color':colors[i]}, opacity=.5), row=3,col=2)
        #tempo
        fig.add_trace(go.Box(y=artist_features.tempo, name=artist, fillcolor=colors[i], line={'color':colors[i]}, opacity=.5), row=3,col=3)
